...
- [ ] **Full-time-employee content (HR/benefits).** The knowledge base currently leans toward intern needs. Missing or thin: health-insurance plan comparison and deductible/network details, equity vesting schedule, 401(k) match and waiting period, full PTO policy, parental leave, performance review cadence, promotion process. Someone with FTE access should fill these in.
- [ ] **Full-time-employee infra & routing details.** What an intern uses is a subset of what an FTE touches. Missing: full Portkey workspace setup beyond just "get a key" (model catalog management, virtual keys for different teams, fallback/retry routing, budget controls), production infra access (VPN / Tailscale subnets for private services, internal API gateways, service routing for prod systems), CI/CD pipelines, observability/monitoring tooling (Datadog/Grafana/Sentry/whatever's in use), on-call rotation and escalation paths, internal DNS / `golinks`. Anything that goes beyond "log in to the tool" and into "how the team actually wires things together."
